...
public class ChatServer {
    ...
    public synchronized void sendToUser(String username, String message, byte[] file) {
        ...
        if (out != null) {
            ...
            if (file != null) {
                ...
                storeFile(file, fileName, username, sender);
    ...
    public synchronized void sendToChannel(String channel, String message, String sender, byte[] file) {
        ...
        for (PrintWriter client : channelClients) {
            ...
            if (file != null) {
                String fileName = "file_" + System.currentTimeMillis() + ".dat";
                storeFile(file, fileName, "#" + channel, sender);
    ...
    public synchronized void sendChannelHistory(String channel, PrintWriter out) {
        try (Connection conn = db.getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                 "SELECT u.username AS sender, m.message, m.file, m.file_name, m.timestamp FROM messages m JOIN users u ON m.sender_id = u.id WHERE m.destination = ? ORDER BY m.timestamp")) {
            stmt.setString(1, "#" + channel);
            ResultSet rs = stmt.executeQuery();
            while (rs.next()) {
                ...
                if (file != null && fileName != null) {
                    out.println(cryptoService.encrypt("HISTORY_FILE:" + channel + ":" + sender + ":" + fileName + ":" + Base64.getEncoder().encodeToString(file)));
                }
            }
    ...
    public synchronized void sendChatHistory(String user, PrintWriter out) {
        try (Connection conn = db.getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                 "SELECT u.username AS sender, m.message, m.file, m.file_name, m.timestamp FROM messages m JOIN users u ON m.sender_id = u.id WHERE (m.destination = ? AND u.username = ?) OR (m.destination = ? AND u.username = ?) ORDER BY m.timestamp")) {
            ...
            while (rs.next()) {
                ...
                if (file != null && fileName != null) {
                    out.println(cryptoService.encrypt("HISTORY_FILE:" + user + ":" + sender + ":" + fileName + ":" + Base64.getEncoder().encodeToString(file)));
                }
            }
    ...
    public List<Map<String, String>> getRegisteredUsers() {
        ...
        try (Connection conn = db.getConnection();
             Statement stmt = conn.createStatement();
             ResultSet rs = stmt.executeQuery("SELECT username, email, ip_address, photo FROM users")) {
            while (rs.next()) {
                Map<String, String> user = new HashMap<>();
                user.put("username", rs.getString("username"));
                user.put("email", rs.getString("email"));
                user.put("ip_address", rs.getString("ip_address"));
    ...
    private void logMessage(String sender, String destination, String message, byte[] file) {
        ...
        String sql =
        "INSERT INTO messages (sender_id, destination, message, file, file_name) " +
        ...
        try (Connection conn = db.getConnection();  
            ...
            stmt.setString(4, file != null ? "file_" + System.currentTimeMillis() + ".dat" : null);
            stmt.setString(5, sender);
            int rowsAffected = stmt.executeUpdate();
            if (rowsAffected == 0) {
                log("Error: No se pudo registrar el mensaje, usuario " + sender + " no encontrado.");
            } else {
                log("Mensaje guardado en la base de datos para " + destination);
            }
        } catch (SQLException e) {
            log("Error al guardar mensaje en la base de datos: " + e.getMessage());
        }
    }

    public void storeFile(byte[] file, String fileName, String destination, String sender) {
        
        String sql = "INSERT INTO messages (sender_id, destination, message, file, file_name) " +
                 "SELECT id, ?, ?, ?, ? FROM users WHERE username = ?";
        ...
        try (Connection conn = db.getConnection();
            ...
            stmt.setString(1, destination);
            stmt.setString(2, "Archivo almacenado: " + fileName);
            stmt.setBytes(3, file);
            stmt.setString(4, fileName);
            stmt.setString(5, sender);
            stmt.executeUpdate();
            log("Archivo " + fileName + " guardado en la base de datos para " + destination);
        } catch (SQLException e) {
            log("Error al guardar archivo en la base de datos: " + e.getMessage());
        }
    ...
    public synchronized void sendFileToUser(String username, String sender, String originalFileName, byte[] file) {
        PrintWriter out = clients.get(username);
        if (out != null) {
            storeFile(file, originalFileName, username, sender);
            String fileMessage = "FILE|" + username + "|" + sender + "|" + originalFileName + "|" + Base64.getEncoder().encodeToString(file);
            out.println(cryptoService.encrypt(fileMessage));
    ...
    public synchronized void sendFileToChannel(String channel, String sender, String originalFileName, byte[] file) {
        Set<PrintWriter> channelClients = channels.getOrDefault(channel, new HashSet<>());
        String fileMessage = "FILE|#" + channel + "|" + sender + "|" + originalFileName + "|" + Base64.getEncoder().encodeToString(file);
        storeFile(file, originalFileName, "#" + channel, sender); 
        for (PrintWriter client : channelClients) {
            client.println(cryptoService.encrypt(fileMessage));
        }
    ...
    protected void saveFileOnServer(byte[] file, String fileName, String destination) {
    ...
    protected String getUsername(PrintWriter out) {
        for (Map.Entry<String, PrintWriter> entry : clients.entrySet()) {
            if (entry.getValue().equals(out)) {
                return entry.getKey();
            }
