...
public class DatabaseConnector implements DatabaseConnection {
    ...
    private void createTablesIfNotExist() {
        try (Connection conn = dataSource.getConnection();
            ...
            stmt.execute("CREATE TABLE IF NOT EXISTS users (id INT AUTO_INCREMENT PRIMARY KEY, username VARCHAR(50) UNIQUE NOT NULL, email VARCHAR(100) UNIQUE NOT NULL, password VARCHAR(100) NOT NULL, photo BLOB, ip_address VARCHAR(45))");
            ...
            stmt.execute("CREATE TABLE IF NOT EXISTS messages (id INT AUTO_INCREMENT PRIMARY KEY, sender_id INT, destination VARCHAR(255), message TEXT, file BLOB, file_name VARCHAR(255), timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (sender_id) REFERENCES users(id))");
